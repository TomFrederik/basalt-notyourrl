...
def load_expert_demo(env_name, data_dir, num_expert_episodes, centroids, combined_memory):
    #TODO change centroids into something else
    ...
    for i, trajectory_name in enumerate(trajectory_names):
        ...
        #TODO actions
        actions = [np.argmin(np.sum((np.array(act['vector'])[None,:] - centroids)**2, axis=1)) for i in range(len(act))]
...
def loss_function(reward, discount_factor, action, best_one_step_target_value, cur_expert_action, cur_q_values, supervised_loss_margin, n_step_reward, best_n_step_target_value, n_step):
...
def pretrain(dataset, q_net, pretrain_steps, batch_size, supervised_loss_margin, lr, weight_decay):
    ...
    while steps < pretrain_steps:
        ...
        (pov, vec), _, _, cur_expert_action, reward, _, idcs, weight = zip([dataset[idx] for idx in batch_idcs])
...
def main(env_name, max_episode_len, model_path, max_env_steps, centroids_path, training_steps_per_iteration,
    ...
    # init memory
    IS_exponent = IS_exponent_0
    combined_memory = CombinedMemory(capacity, n_step, discount_factor, {'agent':agent_p_offset, 'expert':expert_p_offset}, PER_exponent, IS_exponent)
    ...
    # pretrain q_net
    q_net = pretrain(dataset, q_net, pretrain_steps)
